 ...
 * @author Kevin Wayne
 */
public class axg137230_RedBlackBST {
	...
	public class Node {
		...
		private LinkedList<axg137230_Item> list;
		// private axg137230_Item val; // associated data
		private Node left, right; // links to left and right subtrees
		private boolean color; // color of parent link
		private int N; // subtree count

		public Node(double key, axg137230_Item val, boolean color, int N) {
			this.key = (double)key;
			list = new LinkedList<axg137230_Item>();
			list.add(val);
			// this.val = val;
			this.color = color;
			this.N = N;
	...
	public LinkedList<axg137230_Item> get(double key) {
		return get(root, key);
	}

	// value associated with the given key in subtree rooted at x; null if no
	// such key
	private LinkedList<axg137230_Item> get(Node x, double key) {
		while (x != null) {
	...
	public void put(double key, axg137230_Item val) {
	...
	private Node put(Node h, double key, axg137230_Item val) {
		if (h == null)
			return new Node((double)key, val, RED, 1);

		double cmp = (double)key - (double)h.key;
		if (cmp < 0.0d)
	...
	public void delete(double key, axg137230_Item id) {
		if (!contains(key)) {
	...
	private Node delete(Node h, double key, axg137230_Item item) {
		// assert contains(h, key);

		if (((double)key - (double)h.key) < 0.0d) {
			if (!isRed(h.left) && !isRed(h.left.left))
				h = moveRedLeft(h);
			h.left = delete(h.left, key, item);
		} else {
			if (isRed(h.left))
				h = rotateRight(h);
			if (key - h.key == 0 && (h.right == null)) {
				// there is only one item left in the linkedlist
				h.list.remove(item);
				if (h.list.isEmpty()) {
					return null;
				} else {
			...
			if ((double)key - (double)h.key == 0.0d) {
				...
				h.right = delete(h.right, key, item);
		}
		return balance(h);
	}
	...
	public Iterable<Double> keys(double lo, double hi) {
		axg137230_Queue<Double> queue = new axg137230_Queue<Double>();
	...
	// to the queue
	private void keys(Node x, axg137230_Queue<Double> queue, double lo,
			double hi) {
		if (x == null)
			return;
		double cmplo = (double)lo - (double)(x.key);
		double cmphi = (double)hi - (double)(x.key);
		if (cmplo < 0.0d)
			keys(x.left, queue, lo, hi);
		if (cmplo <= 0.0d && cmphi >= 0.0d)
			queue.enqueue(x.key);
		if (cmphi > 0.0d)
	...
	private boolean isBST(Node x, Double min, Double max) {
		if (x == null)
			return true;
		if (min != null && x.key - min <= 0)
			return false;
		if (max != null && x.key - max >= 0)
			return false;
