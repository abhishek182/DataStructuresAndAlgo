 ...
 * @author Kevin Wayne
 */
public class axg137230_LinkedListByName {
	...
	private class Node {
		private Long key;
		private axg137230_RedBlackBST val;
		private Node next;

		public Node(Long key, axg137230_RedBlackBST val, Node next) {
			this.key = key;
			this.val = val;
			this.next = next;
		}
	 ...
	 */
	public axg137230_LinkedListByName() {
	}
	 ...
	 *            the value
	 */
	public void put(Long name, axg137230_RedBlackBST val) {
		for (Node x = first; x != null; x = x.next)
			if (name.equals(x.key)) {
				x.val = val;
				return;
			}
		first = new Node(name, val, first);
		N++;
	}
	...
	public void delete(Long key, double price, axg137230_Item item) {
		first = delete(first, key, price, item);
	}

	// delete key in linked list beginning at Node x
	// warning: function call stack too large if table is large
	private Node delete(Node x, Long key, double price, axg137230_Item item) {
		if (x == null)
			return null;
		if (key.equals(x.key)) {
			x.val.delete(price, item);
			if (x.val.size() == 0){
				N--;
				return x.next;
			}else 
		...
		x.next = delete(x.next, key, price, item);
		return x;
	}
